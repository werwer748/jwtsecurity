...
/**
 * @Order: 컴포넌트나 빈의 로드 순서를 정의한다. (같은 타입의 빈이 Autowired 될 때 그 순서를 @Order를 이용해 정의하는 것
 * int범위 내에서 의존성 주입 우선순위를 설정한다(수치가 낮을수록 높다.)
 * 이 때 우선순위를 너무 높이면(= 값이 너무 작으면) 유효한 토큰이라도 인증이 안되고, 우선순위가 너무 낮으면(= 값이 너무 크면) 토큰이 없어도 통과되기 때문에
 * 적당한 값으로 설정한다.
 */
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
     ...
     *
     * 인증 토큰의 details에는 요청을 날린 클라이언트 또는 프록시의 IP 주소와 세션 ID를 저장하는데, 이 부분은 필요하지 않다면 삭제해도 된다.
     */
    @Override
    ...
    /**
     * HTTP 요청의 헤더에서 Authorization값을 찾아서 Bearer로 시작하는지 확인하고 접두어를 제외한 토큰값으로 파싱한다.
    ...
    /**
     * 토큰값을 토대로 토큰에 담긴 회원ID와 회원타입을 토대로 스프링 시큐리티에서 사용할 User 객체를 반환한다.
